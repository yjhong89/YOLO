...
def train(config, args):
    ...
    with tf.name_scope('optimizer'):
        ...
        optimizer = get_optimizer(config, args.optimizer_name, learning_rate)
        tf.logging.info('%s optimizer with learning rate %3.7f' % (args.optimizer_name, args.learning_rate))

    vars_update_ops = _get_vars_and_update_ops(yolo.yolo_name)

    train_op = slim.learning.create_train_op(total_loss, optimizer, update_ops=vars_update_ops[1], 
            variables_to_train=vars_update_ops[0], clip_gradient_norm=args.gradient_norm, summarize_gradients=False)

    init_fn = lambda sess : tf.logging.info('%d global steps' % sess.run(global_step))
    
    slim.learning.train(train_op, log_dir, master='', is_chief=(args.task == 0), global_step=global_step,
                number_of_steps=args.steps, init_fn=init_fn, summary_writer=tf.summary.FileWriter(log_dir),
